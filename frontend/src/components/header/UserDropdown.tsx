...
/** Renders initials (e.g. "MH") in a coloured circle when no picture is set. */
function Avatar({
  src,
  firstName,
  lastName,
  ...
  return (
    <span className="flex items-center justify-center w-full h-full rounded-full bg-brand-500 text-white font-semibold text-sm">
      {initials}
    </span>
  );
}
...
export default function UserDropdown() {
  ...
  const shortName = profile?.firstName ?? displayName.split(" ")[0] ?? "…";
